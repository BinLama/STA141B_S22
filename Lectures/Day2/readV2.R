...
function(filename, text = readLines(filename))
{
    ...
    info = lapply(parts, function(x)  matrix(unlist(strsplit(unlist(strsplit(x[-(1:4)], "=")), ",")), , 4, byrow = TRUE))
    i = rep(seq(along = info), sapply(info, nrow))    

    # combine the matrices in info into a single matrix
    info = do.call(rbind, info)
    colnames(info) = c("mac", "signal", "channel", "devType")

    # Repeat the fixed parts as many times as there are detected devices in each line of the text
    # and combine this with the info matrices.
